-- 0007: allow launcher_install and launcher_launch task kinds.
--
-- SQLite cannot alter a CHECK constraint in place, so the tasks table is
-- rebuilt with the extended kind list.

CREATE TABLE tasks_v9 (
  id TEXT PRIMARY KEY,
  pack_id TEXT REFERENCES packs(id) ON DELETE SET NULL,
  kind TEXT NOT NULL CHECK (kind IN ('resolve','download','index','build','publish','import','cache_gc','tool_install','launcher_install','launcher_launch')),
  title TEXT NOT NULL,
