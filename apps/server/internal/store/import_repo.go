...
func (r *Repository) CreateImportPreview(ctx context.Context, p ImportPreviewRecord) error {
	...
	if err != nil {
		return fmt.Errorf("create import preview: %w", err)
	}
	return nil
}
...
func (r *Repository) GetImportPreview(ctx context.Context, id string) (ImportPreviewRecord, error) {
	...
	if err != nil {
		return ImportPreviewRecord{}, fmt.Errorf("get import preview: %w", err)
	}
	return p, nil
}
...
func (r *Repository) ConsumeImportPreview(ctx context.Context, id, tokenHash, inputHash string, now int64) (ImportPreviewRecord, error) {
	...
	err := r.WithTx(ctx, func(tx *Repository) error {
		...
		if p.TokenHash != tokenHash || p.InputHash != inputHash {
			return ErrConflict
		}
		if p.ConsumedAt.Valid || p.ExpiresAt.Int64 <= now {
			return ErrConflict
		}
		...
		p.ConsumedAt = sql.NullInt64{Int64: now, Valid: true}
		out = p
		return nil
	})
	if err != nil {
