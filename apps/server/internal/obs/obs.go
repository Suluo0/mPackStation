...
type Logger interface { Info(context.Context, string, map[string]any); Error(context.Context, string, error, map[string]any) }
...
type MemoryLogger struct { mu sync.Mutex; Events []Event }
type Event struct { Level, Message string; Err error; Fields map[string]any }
func (l *MemoryLogger) Info(_ context.Context, msg string, fields map[string]any) { l.add(Event{Level: "info", Message: msg, Fields: clone(fields)}) }
func (l *MemoryLogger) Error(_ context.Context, msg string, err error, fields map[string]any) { l.add(Event{Level: "error", Message: msg, Err: err, Fields: clone(fields)}) }
...
func WithRequestID(ctx context.Context, id string) context.Context { if id == "" { var b [8]byte; if _, err := rand.Read(b[:]); err == nil { id = hex.EncodeToString(b[:]) } }; return context.WithValue(ctx, requestIDKey{}, id) }
