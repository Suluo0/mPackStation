...
func (s *Store) Put(r io.Reader) (string, int64, error) {
	f, err := os.CreateTemp(s.temp, "blob-")
	if err != nil {
		return "", 0, err
	}
	tmp := f.Name()
	...
	rel := filepath.Join("blobs", digest[:2], digest)
	dest, err := s.policy.Resolve(rel)
	if err != nil {
		return "", 0, err
	}
...
func (s *Store) Open(digest string) (*os.File, error) {
	if len(digest) != 40 {
		return nil, errors.New("invalid sha1 digest")
	}
	p, err := s.policy.Resolve(filepath.Join("blobs", digest[:2], digest))
	if err != nil {
		return nil, err
	}
	return os.Open(p)
