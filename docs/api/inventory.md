...
> **历史资料说明**：本文件记录的是一次阶段性盘点，数量和“已接/未接”会随代码变化而过期。当前判断以 [`implementation-status.md`](./implementation-status.md)、[`integration-matrix.md`](./integration-matrix.md) 和日期化审计为准；新增接口必须同步更新状态表。
