...
## P7 联调补充：前后端真实接口闭环（当前目标，2026-08-30 新增）

本环节纳入本次 P0–P7 交付范围，不归入 P9 视觉收工。以用户当前 Dashboard 接入改动为基础，保留页面设计，替换可接入页面中的 mock 数据和假成功动作。

1. 统一前端 HTTP 封装、领域 API adapter、zod 请求/响应契约和错误处理。
2. 接入 Pack 列表/详情、模组搜索与管理、依赖/冲突/健康、内容 revision、任务书、构建/产物/发布。
3. 导入采用 inspect/confirm；允许入队时 packId 为 null，依靠任务状态确认结果。
4. 内容/任务书保存携带 If-Match；写操作保留 token、幂等和错误语义，不通过放宽后端安全验证解决对接问题。
5. 后端尚无能力的设置、真实平台发布等入口明确显示不可用或真实错误，不生成模拟成功结果。
6. 使用独立测试数据目录进行真实服务、前端 adapter 和浏览器联调；分别记录请求、状态码、响应断言和刷新后持久性。

验收必须区分：已接线、已编译、真实请求已通过、尚未验证/后端阻断。不得用前端构建或后端单元测试代替联调。
